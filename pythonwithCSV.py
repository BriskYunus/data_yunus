...
categories = [] # these are the headers in the CSV file
installs = [] # this is the install row
...
with open('data/googeplaystore.csv') as csvfile:
	...
	for row in reader:
		...
		if line_count is 0: # this iwll be text, not data
			...
			# grab the ratings and push them into the rating array
			ratingsData = row[2]
			ratingsData = ratingsData.replace("NaN", "0")
			ratings.append(float(ratingsData)) # int will turn string of (piece of text) into a number
			# print ('pushing ratings data into the ratings array')
			installData = row[5]
			installData = installData.replace(",", "") # get rid of commas

			installs.append(np.char.strip(installData, "+")) # get rid of the trailing "+"
			line_count += 1
